...
aws elb create-load-balancer-policy --load-balancer-name ${ELB_NAME} --policy-name ${APP_NAME}-elb --policy-type-name ProxyProtocolPolicyType --policy-attributes AttributeName=ProxyProtocol,AttributeValue=true
aws elb set-load-balancer-policies-for-backend-server --load-balancer-name ${ELB_NAME} --instance-port 80 --policy-names ${APP_NAME}-elb
aws elb describe-load-balancers --load-balancer-name ${ELB_NAME}
